...
class CategoryDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        ->editColumn('active', function(Category $category) {
            return view('backend.includes.tables.checkbox-status', ['value' => $category->active, 'route' => routeHelper('categories.active.toggle')."?category_id=$category->id"]);
        })
        ->filterColumn('active', function ($query, $keywords) {
            $active = (str_replace(['TRUE','true','YES','yes','ACTIVE','active'], '', $keywords) != $keywords);
            return $query->where('active', $active);
        })
        ->addColumn('check', 'backend.includes.tables.checkbox')
        ->editColumn('action', function(Category $category) {return view('backend.categories.actions', ['id' => $category->id])->render();})
        ->rawColumns(['action', 'check', 'name', 'subs','active']);
    }
