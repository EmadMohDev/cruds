...
class RoleDataTable extends DataTable
{
    ...
    public function html()
    {
        ...
        ->pageLength(5)
        ->buttons([
            Button::make()->text('<i class="fa fa-plus"></i> <span class="hidden" data-yajra-href="'.routeHelper('roles.create').'"></span>')->addClass('btn btn-info show-modal-form '. (canUser("routes.assign") ? "" : "remove-hidden-element"))->titleAttr(trans('menu.create-row', ['model' => trans('menu.role')])),
            Button::make()->text('<i class="fas fa-trash"></i>')->addClass('btn btn-danger multi-delete '. (canUser("routes-multidelete") ? "" : "remove-hidden-element"))->titleAttr(trans('buttons.multi-delete')),
            Button::make('excel')->text('<i class="fas fa-download"></i>')->addClass('btn btn-success')->titleAttr(trans('buttons.export-file')),
