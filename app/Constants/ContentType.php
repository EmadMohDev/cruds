...
class ContentType {
    ...
    public static function validaionHandler(int $type_id) :array
    {
        ...
        switch ($type_id) {
            ...
            case self::VIDEO:
                ...
                $validations['data'] = "required_without:id|mimes:".implode(',', fileExtensions());
                break;
    ...
    public function requestHandler(int $type_id, array $request) :array
    {
        ...
        switch ($type_id) {
            ...
            case self::IMAGE:
                [$width, $height] = getimagesize($request['data']);
                $response['data'] = $this->uploadImage($request['data'], 'contents', $width, $height);
                break;

            case self::AUDIO:
            case self::FILE:
            case self::VIDEO:
                $this->createDataObject($response, $request, $type_id);
                break;
        }
        return $response;
    }
    ...
    protected function createDataObject(&$response, $request, $type_id)
    {
        $data = $this->uploadImage($request['data'], 'contents', null, null);
        foreach (config('languages') as $lang)
            $response['data'][$lang] = $data;

        if (! isset($request['video_thumb']) && $type_id == self::VIDEO)
            $response['video_thumb'] = $this->videoImagePreview($data, 'contents');
    }
}
