...
function createEmail(array $email) :Email
{
    DB::beginTransaction();
        ...
        $users = User::whereIn('email', $email['to'])->when(isset($email['cc']), function($query) use($email) {
                            $query->orWhereIn('email', $email['cc']);
                        })->pluck('id');

        $new_email->load('notifier');
        foreach ($users as $id) {
            broadcast( new NewEmail( $id, $new_email ) );
        }
    DB::commit();
