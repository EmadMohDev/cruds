$(function () {
    ...
    $('body').on('click', '.multi-delete', function (e) {
        ...
        if (ids.length == 0) {
            Swal.fire({ title: SWAL_FAILED_TITLE, icon: 'warning', showConfirmButton: false });
        } else {
            Swal.fire({
                title: SWAL_TITLE,
                text: SWAL_MESSAGE,
    ...
    $("body").on('click', '.copy, .copy-url', function(e) {
        ...
        $(this).parent().append(`<input id="copy-this-path-input" value='${text}'>`);
